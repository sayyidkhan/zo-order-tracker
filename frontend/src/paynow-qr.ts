...
function emvNestedField(id: string, subfields: Array<{ id: string; value: string }>) {
  const nested = subfields.map((field) => emvField(field.id, field.value)).join("");
  return emvField(id, nested);
}
...
function crc16CcittFalse(payload: string) {
  ...
  for (let index = 0; index < payload.length; index += 1) {
    ...
    if (code > 255) {
      throw new RangeError("PayNow payload contains non-ASCII characters.");
    }

    const tableIndex = (code ^ (crc >> 8)) & 0xff;
    crc = CRC16_TABLE[tableIndex] ^ (crc << 8);
  }
...
export function normalizePaynowMobileForPayload(value: string) {
  ...
  return `+65${localDigits}`;
}

export function buildPayNowPayload(options: { mobileNumber: string; merchantName: string }) {
  const proxyValue = normalizePaynowMobileForPayload(options.mobileNumber);
  if (!proxyValue) {
    return null;
  }

  const merchantName = options.merchantName.trim().slice(0, 25) || "NA";
  const expiry = formatExpiryDate(new Date(Date.now() + 365 * 24 * 60 * 60 * 1000));
  const paynowSubfields = [
    { id: "00", value: "SG.PAYNOW" },
    { id: "01", value: "0" },
    { id: "02", value: proxyValue },
    { id: "03", value: "1" },
    { id: "04", value: expiry }
  ];
  ...
  const payloadWithoutCrc = [
    emvField("00", "01"),
    emvField("01", "12"),
    emvNestedField("26", paynowSubfields),
    emvField("52", "0000"),
    emvField("53", "702"),
    emvField("58", "SG"),
  ...
  const checksumInput = `${payloadWithoutCrc}6304`;
  return `${checksumInput}${crc16CcittFalse(checksumInput)}`;
}

export async function generatePayNowQrDataUrl(mobileNumber: string, merchantName: string) {
  const payload = buildPayNowPayload({ mobileNumber, merchantName });
  if (!payload) {
    return null;
  }
